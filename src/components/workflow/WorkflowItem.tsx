...
import { Star, MoreVertical, ChevronUp, ChevronDown, Edit, Copy } from "lucide-react";
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
      ...
      <div className="border-b border-gray-200">
        <div className="flex items-center py-4 px-2">
          <div className="flex items-center space-x-4 w-1/4">
            <Star 
              className={`cursor-pointer ${isFavorite ? "fill-yellow-400 text-yellow-400" : "text-gray-300"}`} 
          ...
          <div className="w-1/4 text-sm text-gray-600 truncate">{description}</div>
          ...
          <div className="ml-8 mb-4 border-l-2 border-gray-200 pl-4">
            {runHistory.map((run, index) => (
              <div key={index} className="flex items-center space-x-4 py-2">
                ...
                <div className={`text-xs px-2 py-0.5 rounded ${run.status === 'Passed' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
                  {run.status}
                </div>
                <Button variant="ghost" size="icon">
